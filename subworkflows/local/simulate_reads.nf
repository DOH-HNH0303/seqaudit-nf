...
workflow SIMULATE_READS {
    ...
    // Debug: Print what's in the genomes channel
    ch_genomes.view { meta, genome ->
        "Sample: ${meta.id}, ONT: ${meta.ont_reads}, PacBio: ${meta.pacbio_reads}, Illumina: ${meta.illumina_reads}"
    }

    // Create separate channels for each technology using filter
    ch_ont_input = ch_genomes.filter { meta, genome ->
        log.info "Checking ONT for ${meta.id}: ${meta.ont_reads} > 0 = ${meta.ont_reads > 0}"
        return meta.ont_reads > 0
    }

    ch_pacbio_input = ch_genomes.filter { meta, genome ->
        log.info "Checking PacBio for ${meta.id}: ${meta.pacbio_reads} > 0 = ${meta.pacbio_reads > 0}"
        return meta.pacbio_reads > 0
    }

    ch_illumina_input = ch_genomes.filter { meta, genome ->
        log.info "Checking Illumina for ${meta.id}: ${meta.illumina_reads} > 0 = ${meta.illumina_reads > 0}"
        return meta.illumina_reads > 0
    }

    ch_all_genomes = ch_genomes

    // Debug: Count samples going to each technology
    ch_ont_input.count().view { "ONT samples: $it" }
    ch_pacbio_input.count().view { "PacBio samples: $it" }
    ch_illumina_input.count().view { "Illumina samples: $it" }
    ...
    if (params.ont_simulator == 'pbsim3') {
        ...
        Channel.fromPath(params.ont_model_url).view()
    ...
    if (params.pacbio_simulator == 'pbsim3') {
        ch_pacbio_input = ch_genomes.filter { meta, genome ->
        log.info "Checking PacBio for ${meta.id}, ${genome}: ${meta.pacbio_reads} > 0 = ${meta.pacbio_reads > 0}"
        return meta.pacbio_reads > 0
        }
        log.info "Using PBSIM3 for PacBio simulation"
        ...
        log.info "No PacBio simulator specified or unrecognized: ${params.pacbio_simulator}"
        ch_all_pacbio_reads = Channel.empty()
    }
    ...
    ch_manifest_input = ch_all_genomes
